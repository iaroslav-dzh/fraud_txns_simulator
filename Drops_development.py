...
class DropTimeHandler:
    ...
    def get_txn_time(self, receive, in_txns, lag_interval=1440):
        ...
        if self.in_txns == self.in_lim:
            ...
            lag_interval = self.lag_interval + time_delta
            ...
            self.last_unix = derive_from_last_time(last_txn_unix=self.last_unix, lag_interval=lag_interval)
            self.start_unix = self.last_unix
            return self.last_unix
        ...
        self.last_unix = derive_from_last_time(last_txn_unix=self.last_unix, lag_interval=time_delta)
